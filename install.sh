...
health_check_listen_ports() {
    ...
    done < <(grep -E '^listen[[:space:]]*=' "$config_file" 2>/dev/null || true)
    fi

    if [ ${#ports[@]} -eq 0 ]; then
    echo -e "${YELLOW}未发现监听端口配置${RESET}"
    return 1
    fi
    ...
    if ss -tuln 2>/dev/null | grep -q ":${port}[[:space:]]"; then
            echo -e "${GREEN}✓ 端口 $port 正在监听${RESET}"
            port_ok=$((port_ok + 1))
    else
            echo -e "${RED}✗ 端口 $port 未监听${RESET}"
...
health_check_panel_port() {
    if [ ! -f "$PANEL_SERVICE_FILE" ]; then
    return 0
    fi

    local panel_port
    panel_port="$(grep -E 'PANEL_PORT=' "$PANEL_SERVICE_FILE" | head -n1 | sed 's/.*PANEL_PORT=\([0-9]*\).*/\1/')"
    if [ -z "$panel_port" ]; then
    panel_port="4794"
    fi

    if ss -tuln 2>/dev/null | grep -q ":${panel_port}[[:space:]]"; then
    echo -e "${GREEN}✓ 面板端口 $panel_port 正在监听${RESET}"
    return 0
    fi

    echo -e "${RED}✗ 面板端口 $panel_port 未监听${RESET}"
    return 1
}

health_check_config() {
    if [ ! -s "$CONFIG_FILE" ]; then
    echo -e "${RED}✗ 配置文件不存在或为空：$CONFIG_FILE${RESET}"
    return 1
    fi

    if grep -q '^\[\[endpoints\]\]' "$CONFIG_FILE" 2>/dev/null; then
    echo -e "${GREEN}✓ 配置文件包含 endpoints${RESET}"
    else
    echo -e "${YELLOW}⚠ 配置文件未找到 endpoints${RESET}"
    fi
...
full_health_check() {
    echo -e "${GREEN}===== 健康检查 =====${RESET}"
    local fail=0

    health_check_service realm || fail=1
    if [ -f "$PANEL_SERVICE_FILE" ]; then
    health_check_service realm-panel || fail=1
    fi

    health_check_config || fail=1
    health_check_listen_ports "$CONFIG_FILE" || fail=1
    health_check_panel_port || fail=1

    if [ "$fail" -eq 0 ]; then
    echo -e "${GREEN}健康检查通过${RESET}"
    else
    echo -e "${YELLOW}健康检查完成，存在异常项${RESET}"
    fi
}
...
EOF
    ...
    run_or_die "创建临时目录失败: $TMP_DIR" mkdir -p "$TMP_DIR"
    cd "$TMP_DIR" || {
        log_error "无法进入临时目录: $TMP_DIR"
        exit 1
    }
    rm -f realm.tar.gz realm
    run_or_die "下载失败: $url" curl -L -o realm.tar.gz "$url"
    run_or_die "解压失败: realm.tar.gz" tar -xzf realm.tar.gz
    if [ ! -f "realm" ]; then
        echo -e "${RED}解压后未找到 realm 可执行文件。${RESET}"
        exit 1
    fi
    run_or_die "安装 realm 可执行文件失败" mv realm "$REALM_BIN"
    chmod +x "$REALM_BIN"
...
export_rules() {
    ...
    if ! tar -czf "$OUT" -C "$(dirname "$CONFIG_FILE")" "${FILES_TO_BACKUP[@]}"; then
        log_error "导出失败：打包配置文件出错"
        return 1
    fi
    if [ -s "$OUT" ]; then
...
import_rules() {
    ...
    if [[ "$IN" == *.tar.gz ]]; then
        ...
        if ! tar -xzf "$IN" -C "$(dirname "$CONFIG_FILE")"; then
            log_error "导入失败：解压备份文件出错"
            return 1
        fi
    else
...
main_menu() {
    ...
    case "$OPT" in
            ...
            18) require_installed && full_health_check ;;
            *) echo -e "${RED}无效选项。${RESET}" ;;
    esac
    done
}
