...
- IPv4 / IPv6 监听与转发支�?- 规则冲突检测：
  - 配置内端口冲突检�?  - 系统端口占用检测（ss/netstat�?- 规则导入 / 导出
- 定时自动备份（cron�?- FTP / SFTP 远程自动备份
...
- 依赖（脚本会尽量自动安装/提示）：
  - `curl`、`tar`、`systemctl`
  - 端口检测：优先 `ss`，否�?`netstat`
  - cron：`cron/crond/crontab`（用于定时备份）
- Web 面板额外依赖（脚本会自动处理）：
  - Rust/cargo（通过 rustup 安装�?  - `build-essential/pkg-config/libssl-dev` �?`Development Tools/openssl-devel`
...
10. 查看日志
11. 查看配置
12. 一键导出所有规�?13. 一键导入所有规�?14. 添加/删除定时备份任务
15. 自动备份到FTP/SFTP
16. Realm 面板管理
17. 端口流量狗管�?0.  退�?```
...
- 自动识别 CPU 架构（x86_64 / arm / arm64�?- 自动识别 libc（glibc / musl�?- 自动下载 **GitHub 最新版�?Realm**
- 自动创建并启�?systemd 服务（`realm.service`�?
### 卸载 Realm
...
- 是否与现有规则冲突（配置端口冲突�?- 是否被系统占用（ss/netstat 检测）
...
- 登录鉴权（cookie session�?- 可视化管理规则：添加 / 编辑 / 删除 / 启停
- 一键删除，批量添加，一键导�?导入
- 带宽限速（tc�?- 月度流量配额与自动重�?- 通知告警配置（Telegram / 企业微信�?- 远程节点管理（多级链路转�?/ API 令牌�?- 计费模式（单�?/ 双向�?- 端口冲突检测（同端口禁止重复）
- 可在面板设置里修改：
  - 管理账户（用户名 / 密码�?  - PC/手机端背景图 URL
  - 远程节点�?API 令牌
- 规则保存到：
...
> 注意：面板会�?*启用的规�?*写入 realm 配置，禁用规则不会写�?endpoints�? 
> 若启用规则为空，为避�?Realm �?endpoints，面板会写入一�?keepalive 的本地占位规则（127.0.0.1:65534）�?
---
...
- 📊 **流量监控**: 基于 nftables 的精确流量统计（支持双向/单向模式�?- 🚀 **带宽限�?*: 使用 tc 流量控制实现端口级别限�?- 📈 **流量配额**: 月度流量配额管理，支持自动重�?- 🔔 **通知告警**: 支持 Telegram 和企业微信通知
...
### Q1：修改规则后需要重启吗�?
不需要手动重启。无论是脚本修改还是面板操作，都会自动重�?Realm 使之生效�?
### Q2：支�?UDP 吗？
...
- 如果�?**配置冲突**（已有规则同端口），必须换端�?- 如果�?**系统占用**（ss/netstat 检测到），脚本会提示是否仍要使用该端口（不建议�?
### Q5：面板无法访问？
...
2. 确认端口监听�?   ```bash
   ss -lntp | grep 4794
   ```
3. 检查服务器安全�?防火墙放�?`4794/tcp`

---

## 安全建议

- **强烈建议**：安装面板后第一时间修改默认密码（面板设�?�?管理账户�?- 建议通过防火�?安全组限制面板端口仅允许可信 IP 访问
