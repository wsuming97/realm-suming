...
- Realm 配置: `/etc/realm/config.toml`（自动生成/刷新）

**注意事项**:
- 面板将启用的规则写入 realm 配置，禁用规则不会写入 endpoints
- 若启用规则为空，面板会写入一个 keepalive 占位规则（`127.0.0.1:65534`）

### 技术特点
- 面板使用 Rust 语言（Axum 框架），需要编译或下载预编译二进制
- 使用 iptables 自定义链（REALM_IN/REALM_OUT）统计流量
- 配置格式：TOML
- 支持 AMD64 和 ARM64 架构
...
├── README.md             # 中文文档
├── README_EN.md          # 英文文档
├── port-traffic-dog-README.md  # 端口流量狗文档
...
4. **规则备注**: 清晰的备注功能
5. **端口流量犬**: 统计端口流量、控制限速限流、通知
6. **直观配置系统 MPTCP**: 清晰展示 MPTCP 界面
7. **网络链路脚本**: 测试链路延迟、带宽、稳定性、路由情况
8. **一键导出/导入**: 压缩包迁移
9. **一键识别批量导入**: 自写 realm 规则配置
10. **智能检测**: 自动检测系统架构、端口冲突、连接可用性
11. **完整卸载**: 分阶段全面清理
12. **原生 Realm 全功能**: 同步支持最新版 realm
...
2. **纯脚本**: 无需编译，部署简单
3. **社区活跃**: 766 Stars，持续维护更新
4. **网络工具**: 内置链路测试、速度测试

### 使用场景建议
- **选择 hia-realm**: 需要 Web 可视化管理、远程备份
